  ...
  <div class="flex-1 flex items-center justify-center p-8 bg-bg-primary">
    <div class="w-full max-w-[360px] flex flex-col gap-8">
      ...
      <div class="flex flex-col gap-3">
        ...
        <h1 class="serif-italic text-4xl text-bg-dark leading-tight">Log in</h1>
        <p class="font-sans text-sm text-text-secondary">Enter your credentials to access your dashboard.</p>
      </div>

      <form onsubmit={handleSubmit} class="flex flex-col gap-5">
        <div class="flex flex-col gap-2">
          <label for="email" class="mono-label text-gray-400">Account Email</label>
          <input
            id="email"
            type="email"
            bind:value={email}
      ...
      <div class="flex flex-col items-center gap-2 mt-2">
        <p class="font-sans text-sm text-text-secondary">New to the ecosystem?</p>
        <a href="/auth/register" class="mono-label text-gray-400 hover:text-[#4338ca] transition-colors no-underline border-b border-transparent hover:border-[#4338ca] pb-0.5">
          Create account →
        </a>
      </div>
