...
import type { EventOccurrence, ImpactWindow, EntryExitRule } from "$lib/types/pipeline"
...
export const POST: RequestHandler = async ({ request }) => {
	...
	if (!Array.isArray(b.occurrences) || b.occurrences.length === 0) {
		return json({ error: "occurrences must be a non-empty array" }, { status: 400 })
	}

	if (!Array.isArray(b.impact_windows) || b.impact_windows.length === 0) {
		return json({ error: "impact_windows must be a non-empty array" }, { status: 400 })
	}
	...
	const startingCapital =
		typeof b.starting_capital === "number" && b.starting_capital > 0
			? b.starting_capital
			: 10_000

	const result = runSimulation(
		b.occurrences as EventOccurrence[],
		b.impact_windows as ImpactWindow[],
		rule,
		startingCapital
	)
